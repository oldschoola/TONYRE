...
{
	// if (Script::GetInteger(Crc::ConstCRC("use_bike_physics")))
	// {
		// return Script::GetStructure(Crc::ConstCRC("bike_physics"));
	// }
	// else if (Script::GetInteger(Crc::ConstCRC("use_walk_physics")))
	// {
		// return Script::GetStructure(Crc::ConstCRC("skater_physics"));
	// }
	// else
	// {
		return Script::GetStructure(Crc::ConstCRC("skater_physics"));
...
#	endif
	...
	if(	( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netctf") ) ||
		( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netking") ) ||
...
float	CSkater::GetStat(EStat stat)
{
	...
	else
	{
		if (m_stat[stat] == -1.0f)
		{
			value =  GetPhysicsFloat(Crc::ConstCRC("Skater_Default_Stats"));
		}
		else
		{
			value =  m_stat[stat];
...
float	CSkater::GetScriptedStat(Script::CStruct *pSS)
{
	...
	{
		Script::CPair diff;	
		if (pSS->GetPair(Crc::ConstCRC("diff"),&diff))
		{
...
{
	uint32 camera_name = Crc::ConstCRC("SkaterCam0");
	if (m_skater_number == 1)
	{
		camera_name = Crc::ConstCRC("SkaterCam1");
	}
...
void CSkater::SetViewMode( EViewMode view_mode )
{
	...
	switch ( m_viewing_mode )
	{
		...
		case GAMEPLAY_SKATER_ACTIVE:
		{
			...
			if (GetPhysicsInt(Crc::ConstCRC("ScreenShotMode")))
			{
						...
						int heap_num, bool should_tristrip ) : m_link_node( this ), m_local_client( local_client )
{
	...
	if (heap_num == 0)
	{
		Obj::CTracker::Instance()->AddAlias(Crc::ConstCRC("Skater"),this);		
	}
	if (heap_num == 1)
	{
	...
	component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERSTATE);
	CreateComponentFromStructure(component_struct);
																  
	if (m_local_client)
	{
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_INPUT);
		CreateComponentFromStructure(component_struct);
		mp_input_component = GetInputComponent();
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERSCORE);
		CreateComponentFromStructure(component_struct);
		mp_skater_score_component = GetSkaterScoreComponent();

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERMATRIXQUERIES);
		CreateComponentFromStructure(component_struct);

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_TRICK);
		CreateComponentFromStructure(component_struct);
		mp_trick_component = GetTrickComponent();
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERPHYSICSCONTROL);
		CreateComponentFromStructure(component_struct);
		mp_skater_physics_control_component=GetSkaterPhysicsControlComponent();

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERCOREPHYSICS);
		CreateComponentFromStructure(component_struct);
		mp_skater_core_physics_component = GetSkaterCorePhysicsComponent();

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERROTATE);
		CreateComponentFromStructure(component_struct);

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERGAP);
		CreateComponentFromStructure(component_struct);
		mp_skater_gap_component = GetSkaterGapComponent();
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERADJUSTPHYSICS);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_TRIGGER);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERFINALIZEPHYSICS);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERCLEANUPSTATE);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_WALK);
		CreateComponentFromStructure(component_struct);
		mp_walk_component = GetWalkComponent();
		
#		ifdef TESTING_GUNSLINGER
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_RIDER);
		CreateComponentFromStructure(component_struct);

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_WEAPON);
		CreateComponentFromStructure(component_struct);
#		endif

		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERLOCALNETLOGIC);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERENDRUN);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERBALANCETRICK);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERLOOPINGSOUND);
		CreateComponentFromStructure(component_struct);

        component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_STATSMANAGER);
		CreateComponentFromStructure(component_struct);
        mp_stats_manager_component = GetStatsManagerComponent();
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_MOVABLECONTACT);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERRUNTIMER);
		CreateComponentFromStructure(component_struct);
	}
	else
	{
		...
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERNONLOCALNETLOGIC);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERCLEANUPSTATE);
		CreateComponentFromStructure(component_struct);
		
		component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERFLOATINGNAME);
		CreateComponentFromStructure(component_struct);
	...
	component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERSTATEHISTORY);
	CreateComponentFromStructure(component_struct);
	mp_skater_state_history_component = GetSkaterStateHistoryComponent();
	
	component_struct->AddChecksum(Crc::ConstCRC("component"), CRC_SKATERFLIPANDROTATE);
	CreateComponentFromStructure(component_struct);
...
// proximtrigger components are deprecated  	
		...
		CObject* p_viewer_cam = CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("viewer_cam"));
		if (p_viewer_cam)
		{
...
// SkaterInit used to be run there --------------> *	
	...
	Script::CStruct* pSkaterInfoParams = Script::GetStructure(Crc::ConstCRC("skater_asset_info"));

	uint32 skeleton_name = Crc::ConstCRC("human");
...
//--------------------------------------------------------
	...
	pStruct->AddChecksum(Crc::ConstCRC("component"), CRC_RIBBON);
    pStruct->AddChecksum(Crc::ConstCRC("bone"), Crc::ConstCRC("Bone_Wrist_L"));
...
//--------------------------------------------------------
	...
	pModelStruct->AddChecksum( NONAME, Crc::ConstCRC("UseModelLights") );
...
//	p_lights->SetPositionPointer(&m_pos);
	...
	if ( m_local_client )
	{
		GetSkaterSoundComponent()->SetMaxSpeed(GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")));
	}
	...
	*/
    ...
    mp_animation_component->PlaySequence(Crc::ConstCRC("default"));
...
void CSkater::UpdateSkaterInfo( Obj::CSkaterProfile* pSkaterProfile )
{
	...
	m_pushStyle = pSkaterProfile->GetChecksumValue( (Crc::ConstCRC("pushstyle")) );
	m_isGoofy = pSkaterProfile->GetChecksumValue( (Crc::ConstCRC("stance")) ) == ( Crc::ConstCRC("goofy") );
	m_isPro = pSkaterProfile->IsPro();
	m_displayName = pSkaterProfile->GetDisplayName();
	
	// needed for playing correct streams
	pSkaterProfile->GetInfo()->GetInteger( Crc::ConstCRC("is_male"), &m_isMale, Script::ASSERT );

	const char* pFirstName;
	pSkaterProfile->GetInfo()->GetText( Crc::ConstCRC("first_name"), &pFirstName, Script::ASSERT );
...
void CSkater::ResetAnimation()
{
	...
	mp_model_component->HideGeom( Crc::ConstCRC("board"), false, IsLocalClient() );
	
	/*
	// Make sure all atomics are visible
	// (GJ:  Not really sure why the previous board unhiding is still necessary)
	pModelComponent->HideGeom((Crc::ConstCRC("all")), false, IsLocalClient());
...
// so it would never get run.
	...
	if (walk || in_cleanup)
	{
		...
		if (walk)
		{
			p_params->AddChecksum(NO_NAME, Crc::ConstCRC("Walk"));
		}
		if (in_cleanup)
		{
			p_params->AddChecksum(NO_NAME, Crc::ConstCRC("InCleanup"));
		}
	}
	
	mp_script->SetScript(Crc::ConstCRC("SkaterInit"), p_params, this);
	if( IsLocalClient())
	{
		mp_script->Update();
	}
...
void CSkater::ResetPhysics(bool walk, bool in_cleanup)
{
	...
	if (mp_skater_physics_control_component->IsWalking())
	{
		...
		p_params->AddChecksum(Crc::ConstCRC("NewScript"), Crc::ConstCRC("OnGroundAi"));
		JumpToScript(Crc::ConstCRC("Switch_OnGroundAI"), p_params);
		delete p_params;
	}
...
bool CSkater::SkaterEquals( Script::CStruct* pStructure )
{
	...
	if ( pStructure->ContainsFlag( Crc::ConstCRC("is_pro") ) )
	{
		if ( !m_isPro )
			return false;
	}

	if ( pStructure->ContainsFlag( Crc::ConstCRC("is_custom") ) )
	{
		if ( m_isPro )
			return false;
	}

	if ( pStructure->ContainsFlag( Crc::ConstCRC("is_male") ) )
	{
		if ( !m_isMale )
			return false;
	}
	
	if ( pStructure->ContainsFlag( Crc::ConstCRC("is_female") ) )
	{
		if ( m_isMale )
			return false;
	}
	...
	uint32 is_named;
	if ( pStructure->GetChecksum( Crc::ConstCRC("is_named"), &is_named ) )
	{
	...
	uint32 stance;
	if ( pStructure->GetChecksum( Crc::ConstCRC("stance"), &stance ) )
	{
		uint32 myStance = ( m_isGoofy ? Crc::ConstCRC("goofy") : Crc::ConstCRC("regular") );
		if ( stance != myStance )
			return false;
	}
...
bool CSkater::_function0( uint32 Checksum, Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	switch (Checksum)
	{
		...
		case Crc::ConstCRC("GetCameraId"):
		{
			...
			pScript->GetParams()->AddChecksum(Crc::ConstCRC("CameraId"), camera_name);
			break;
		}
...
bool CSkater::_function1( uint32 Checksum, Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	switch (Checksum)
	{
		...
		case 0x1b572abf: // KillSkater 
		{
			...
			bool walk = pParams->ContainsComponentNamed(Crc::ConstCRC("RestartWalking"));
			...
			{
				if (pParams->GetString(Crc::ConstCRC("prefix"),&p_prefix))
				{
		...
		case 0xe62f94a2: // SparksOn
			...
			Script::RunScript( Crc::ConstCRC("sparks_on"), pParams, this );
			mSparksRequireRail=true;
			mSparksOn=true;
		...
		case 0x5436a335: // SparksOff
			...
			Script::RunScript( Crc::ConstCRC("sparks_off"), pParams, this );
			mSparksOn=false;
			mSparksRequireRail=true;
			break;
		...
		case 0xc67c18d7: // BloodOff
		{
			...
			if (!pParams->GetText( Crc::ConstCRC("bone"), &p_bone_name ))
			{
		...
		{
			if (pParams->ContainsFlag(Crc::ConstCRC("set")))
			{
			...
			else if (pParams->ContainsFlag(Crc::ConstCRC("clear")))
			{
		...
		{
			int stat;
			pParams->GetInteger(Crc::ConstCRC("Stat"),&stat,true);
			pScript->GetParams()->AddFloat(Crc::ConstCRC("stat_value"),GetStat((EStat)stat));
			break;
		}
...
//			Script::PrintContents(pParams);
		...
		{
			pScript->GetParams()->AddInteger(Crc::ConstCRC("SkaterNumber"),m_skater_number);
			break;
		}
...
void CSkater::AddCATInfo(Script::CStruct *pStuff)
{
    ...
    for (int i=0; i<Game::vMAX_CREATED_TRICKS; ++i)
    {
        ...
        p_new_trick_struct->AddStructure(Crc::ConstCRC("other"),m_created_trick[i]->mp_other_params);
        p_new_trick_struct->AddArray(Crc::ConstCRC("animations"),m_created_trick[i]->mp_animations);
        p_new_trick_struct->AddArray(Crc::ConstCRC("rotations"),m_created_trick[i]->mp_rotations);

        //Script::PrintContents( p_new_trick_struct );

        p_tricks->SetStructure(i,p_new_trick_struct);
    }
    pStuff->AddArrayPointer(Crc::ConstCRC("Tricks"),p_tricks);
}
...
void CSkater::AddStatGoalInfo(Script::CStruct *pStuff)
{
    ...
    pStuff->AddArrayPointer(Crc::ConstCRC("StatGoals"),p_stat_goals);
}

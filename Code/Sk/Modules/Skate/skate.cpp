...
void		Skate::s_logic_code ( const Tsk::Task< Skate >& task )
{
	...
	else if( ( mdl.GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netking") ) ||
			 ( mdl.GetGameMode()->GetNameChecksum() ==  Crc::ConstCRC("king") ))
	{
...
////////////////////////////////////////////////////////////////////////////////////					
		...
		else
		{
			...
			cam_obj = (Obj::CCompositeObject*) Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("skatercam0"));
			if( cam_obj )
			{
				cam_obj->SetLockOff();
				delete cam_obj;
...
{   	
	...
	Obj::CCompositeObject*p_bailboard = (Obj::CCompositeObject*) Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("bailboard") + skater->GetID());
	if( p_bailboard )
	{
	...
	if( !skater->IsLocalClient() )
	{
		...
		p_params->AddChecksum(Crc::ConstCRC("SkaterId"), skater->GetID());
		Script::RunScript(Crc::ConstCRC("remove_car_from_non_local_skater"), p_params);
		delete p_params;
	}
...
void Skate::DoUpdate()
{
	...
	if ( !Mdl::FrontEnd::Instance()->GamePaused() && !Mdl::CViewer::sGetViewMode() )
	{
		if ( mp_movieManager->IsRolling() )
		{
			...
			{
				Script::RunScript( Crc::ConstCRC("hide_loading_screen") );
			}
		...
		else
		{
			...
			if ( m_lastFrameWasMovieCamera )
			{
				Script::RunScript( Crc::ConstCRC("restore_skater_camera") );
	...
	Script::RunScript( Crc::ConstCRC("Game_Update") );
...
*/
...
#ifdef __PLAT_XBOX__
	int is_changing_levels = Script::GetInteger( Crc::ConstCRC("is_changing_levels"), Script::ASSERT );
	int check_controllers = Script::GetInteger( Crc::ConstCRC("check_for_unplugged_controllers"), Script::ASSERT );
	if ( check_controllers && !is_changing_levels )
	{
		if ( m_first_input_received )
		{
			...
			Front::CScreenElement* p_root_window = p_screen_elem_man->GetElement( Crc::ConstCRC("root_window")  , Front::CScreenElementManager::DONT_ASSERT );
			if ( p_root_window )
			{
				// check that the box isn't already up
				Front::CScreenElement* p_error_box = p_screen_elem_man->GetElement( Crc::ConstCRC("controller_unplugged_dialog_anchor") , Front::CScreenElementManager::DONT_ASSERT );
				if ( !p_error_box )
				{
					...
					for ( int i = 0; i < vMAX_SKATERS; i++ )
					{
						...
						if ( pSkater && pSkater->IsLocalClient() )
						{
							...
							if ( !pHandler->m_Device->IsPluggedIn() )
							{
								...
								if ( m_controller_unplugged_frame_count > Script::GetInteger( Crc::ConstCRC("controller_unplugged_frame_count"), Script::ASSERT ) )
								{
									...
									if ( gamenet_man->InNetGame() )
									{
										...
										pScriptParams->AddChecksum( NONAME, Crc::ConstCRC("leaving_net_game") );
										Skate::LeaveServer();
									...
									pScriptParams->AddInteger( Crc::ConstCRC("device_num"), device_num );
									Script::RunScript( Crc::ConstCRC("controller_unplugged"), pScriptParams );
...
bool ScriptInitSkaterHeaps(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (p_skate->GetGameType()==Crc::ConstCRC("freeskate2p"))
	{

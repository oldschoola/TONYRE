...
void Score::Update()
{
	...
	if( pSkater )
	{
		p_special_bar = p_manager->GetElement(Crc::ConstCRC("the_special_bar_sprite") + pSkater->GetHeapIndex());
	}
	else
	{
		p_special_bar = p_manager->GetElement(Crc::ConstCRC("the_special_bar_sprite"));
	}
...
void Score::TrickTextPulse(int index)
{
	...
	pParams->AddChecksum( Crc::ConstCRC("trick_text_container_id"), Crc::ConstCRC("trick_text_container")  + index);
	pParams->AddChecksum( Crc::ConstCRC("the_trick_text_id"), Crc::ConstCRC("the_trick_text")  + index);
	pParams->AddChecksum( Crc::ConstCRC("the_score_pot_text_id"), Crc::ConstCRC("the_score_pot_text") + index);
	Script::RunScript(Crc::ConstCRC("trick_text_pulse"), pParams);
	delete pParams;
}
...
void Score::TrickTextCountdown(int index)
{
	...
	pParams->AddChecksum( Crc::ConstCRC("the_trick_text_id"), ( Crc::ConstCRC("the_trick_text") ) + index );
	pParams->AddChecksum( Crc::ConstCRC("the_score_pot_text_id"), ( Crc::ConstCRC("the_score_pot_text") ) + index );
	Script::RunScript(Crc::ConstCRC("trick_text_countdown"), pParams );
	delete pParams;
}
...
void Score::TrickTextLanded(int index)
{
	...
	pParams->AddChecksum( Crc::ConstCRC("the_trick_text_id"), ( Crc::ConstCRC("the_trick_text") ) + index );
	pParams->AddChecksum( Crc::ConstCRC("the_score_pot_text_id"), ( Crc::ConstCRC("the_score_pot_text") ) + index );
	Script::RunScript(Crc::ConstCRC("trick_text_landed"), pParams);
	delete pParams;
}
...
void Score::TrickTextBail(int index)
{
	...
	pParams->AddChecksum( Crc::ConstCRC("the_trick_text_id"), ( Crc::ConstCRC("the_trick_text") ) + index );
	pParams->AddChecksum( Crc::ConstCRC("the_score_pot_text_id"), ( Crc::ConstCRC("the_score_pot_text") ) + index );
	Script::RunScript(Crc::ConstCRC("trick_text_bail"), pParams );
	delete pParams;
}
...
void Score::Trigger(char *trick_name, int base_score, Flags flags)
{
	...
	if (!null_trick)
	{
		...
		if (m_currentMult == 1)
		{
			...
			pSkater->BroadcastEvent(Crc::ConstCRC("SkaterEnterCombo"));
		}
...
void Score::SetSpin(int spin_degrees)
{
	...
	spin_position += (int)Obj::GetPhysicsFloat(Crc::ConstCRC("spin_count_slop"));	
	...
	if (m_infoTab[m_currentSpinTrick].spin_mult_index < spin_index)
	{
        ...
        if ( m_infoTab[m_currentSpinTrick].id == Crc::ConstCRC("Ollie") )
        {
            if ( (spin_index%2) == 1 )
            {
		...
		*/
		...
		pSkater->BroadcastEvent(Crc::ConstCRC("SkaterSpinDisplayed"));
	}
}
...
void Score::Land( void )
{
	...
	*/
	...
	{
		if( gamenet_man->InNetGame() && ( pGameMode->GetNameChecksum() != Crc::ConstCRC("netlobby")))
		{
	...
	if ( !pGameMode->IsTrue(Crc::ConstCRC("should_modulate_color")) )
	{
...
void Score::Reset()
{
	...
	*/
	...
	if( ( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netfirefight")) ||
		( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("firefight")))
	{
		m_totalScore = 100;
	}
	else
...
void Score::position_balance_meter(bool state, float value, bool isVertical)
{
	...
	if (state != (bool) is_shown)
	{
		...
		if (state)
		{   
			Script::RunScript(Crc::ConstCRC("show_balance_meter"), pParams );
		}
		else
		{
			Script::RunScript(Crc::ConstCRC("hide_balance_meter"), pParams );
		}
...
void Score::check_high_score(int old_score, int new_score)
{
	...
	if (!skate_mod->GetGameMode()->IsTrue( Crc::ConstCRC("is_career") ))
	{
		return;
	}
...
inline uint32 get_trickname_checksum_from_key_combo( uint32 key_combo, int num_taps = 1 )
{
	...
	Script::CStruct* pTricks = pSkaterProfile->GetTrickMapping( Crc::ConstCRC("trick_mapping") );	
...
inline uint32 get_cat_index_from_key_combo( uint32 key_combo )
{
	...
	if ( !pTricks->GetInteger( key_combo, &cat_trick, Script::NO_ASSERT ) )
	{
		...
		for (size_t i = 0; i < numSpecials; i++ )
		{
			Script::CStruct* pSpecial = pSpecialTricksArray->GetStructure( i );
			int is_cat = 0;
			pSpecial->GetInteger( Crc::ConstCRC("isCat"), &is_cat, Script::NO_ASSERT );
			if ( is_cat != 0 )
			{
				uint32 trickslot;
				pSpecial->GetChecksum( Crc::ConstCRC("trickSlot"), &trickslot, Script::ASSERT );
				if ( trickslot == key_combo )
				{
					...
					pSpecial->GetChecksum( Crc::ConstCRC("trickName"), &cat_trick_checksum, Script::ASSERT );
...
void Score::set_special_is_active ( bool is_active )
{
	if (is_active != m_specialIsActive)
	{
		...
		if( pSkater )
		{
			pSkater->BroadcastEvent(m_specialIsActive ? Crc::ConstCRC("SkaterEnterSpecial") : Crc::ConstCRC("SkaterExitSpecial"));
		}
	}
}
...
//	GetRobotMult();
	...
	int 	kick_in = Script::GetInteger(Crc::ConstCRC("robot_kick_in_count"));							  

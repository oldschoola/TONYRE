...
void CSkaterCorePhysicsComponent::Update()
{
	...
	if (m_vert_air_last_frame != (GetState() == AIR && GetFlag(VERT_AIR) && !GetFlag(SPINE_PHYSICS)))
	{
        ...
        GetObj()->BroadcastEvent(m_vert_air_last_frame ? Crc::ConstCRC("SkaterEnterVertAir") : Crc::ConstCRC("SkaterExitVertAir"));
	}
...
CBaseComponent::EMemberFunctionResult CSkaterCorePhysicsComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("FirstTimeOnThisRail"):
		...
		case Crc::ConstCRC("StartSkitch"):
			start_skitch();
			break;

        // @script | Skitching | Returns True if we are skitching
		case Crc::ConstCRC("Skitching"):
		...
		case Crc::ConstCRC("StopSkitch"):
			StopSkitch();
			break;
			
		// @script | CancelWallpush | Cancels the current wallpush event
		case Crc::ConstCRC("CancelWallpush"):
        ...
        // @flag seconds | time in seconds
        // @flag frames | time in frames
		case Crc::ConstCRC("AirTimeGreaterThan"):
		{
			float t = 0;
		...
		case Crc::ConstCRC("GetAirTime"):
		{
			...
			pScript->GetParams()->AddFloat(Crc::ConstCRC("AirTime"), AirTime * (1.0f / 1000.0f));	
			break;
		}
		
		// @script | GetAirTimeLeft | Puts the amount of air time left before landing, in seconds, into a param called AirTimeLeft
		case Crc::ConstCRC("GetAirTimeLeft"):
		{
			pScript->GetParams()->AddFloat(Crc::ConstCRC("AirTimeLeft"),
		...
		case Crc::ConstCRC("RestoreAutokick"):
		...
		case Crc::ConstCRC("DoCarPlantBoost"):
		{
			GetVel()[Y] = Mth::ClampMin(GetVel()[Y], 0.0f);
			GetVel()[Y] += GetPhysicsFloat(Crc::ConstCRC("Carplant_upward_boost"));	
			...
			front.Normalize();
			GetVel() += front * GetPhysicsFloat(Crc::ConstCRC("Carplant_forward_boost"));
		...
		case Crc::ConstCRC("HandleLipOllieDirection"):
		{
			CControlPad& control_pad = mp_input_component->GetControlPad();
			...
			{
				GetVel() += out * GetPhysicsFloat(Crc::ConstCRC("Lip_side_hop_speed"));
				DUMP_VELOCITY;
				return CBaseComponent::MF_TRUE;
			} 
			else if (control_pad.m_left.GetPressed()
				&& static_cast< int >(control_pad.m_left.GetPressedTime()) > GetPhysicsInt(Crc::ConstCRC("Lip_held_jump_out_time")))
			{
				GetVel() += out * GetPhysicsFloat(Crc::ConstCRC("Lip_side_jump_speed")); 
				DUMP_VELOCITY;
				return CBaseComponent::MF_TRUE;
			} 
			if (control_pad.m_up.GetPressed()
				&& static_cast< int >(control_pad.m_up.GetPressedTime()) > GetPhysicsInt(Crc::ConstCRC("Lip_held_jump_along_time")))
			{
				GetVel() -= along * GetPhysicsFloat(Crc::ConstCRC("Lip_along_jump_speed")); 
				DUMP_VELOCITY;
				return CBaseComponent::MF_TRUE;
			}
			else if (control_pad.m_down.GetPressed()
				&& static_cast< int >(control_pad.m_down.GetPressedTime()) > GetPhysicsInt(Crc::ConstCRC("Lip_held_jump_along_time")))
			{
				GetVel() += along * GetPhysicsFloat(Crc::ConstCRC("Lip_along_jump_speed"));
				DUMP_VELOCITY;
				return CBaseComponent::MF_TRUE;
		...
		case Crc::ConstCRC("OrientToNormal"):
        ...
        case Crc::ConstCRC("FrontTruckSparks"):
		...
		case Crc::ConstCRC("LockVelocityDirection"):
			m_lock_velocity_direction = pParams->ContainsFlag(Crc::ConstCRC("On"));
			break;
			
        // @script | SetRollingFriction | change the rolling friction value
        // @flag Default | use the default value
        // @uparm 0.0 | friction coeff
		case Crc::ConstCRC("SetRollingFriction"):
			pParams->GetFloat(NO_NAME, &m_rolling_friction);
			if (pParams->ContainsFlag(Crc::ConstCRC("Default")))
			{
		...
		case Crc::ConstCRC("SkateInAble"):
			if (pParams->ContainsFlag(Crc::ConstCRC("Lip")))
			{
				float HorizOff = GetPhysicsFloat(Crc::ConstCRC("SkateInAble_LipHorizOffset"));
				float DownOff = GetPhysicsFloat(Crc::ConstCRC("SkateInAble_LipDownOffset"));
				m_col_end = GetPos();
				m_col_end[Y] -= DownOff;
				...
				else
				{
					...
					HorizOff = GetPhysicsFloat(Crc::ConstCRC("SkateInAble_LipExtraCheckHorizOffset"));
					DownOff = GetPhysicsFloat(Crc::ConstCRC("SkateInAble_LipExtraCheckDownOffset"));
			...
			{
				float HorizOff=GetPhysicsFloat(Crc::ConstCRC("SkateInAble_HorizOffset"));
				Mth::Vector a = GetPos() + HorizOff * GetMatrix()[X];
				a[Y] -= GetPhysicsFloat(Crc::ConstCRC("SkateInAble_DownOffset"));
				Mth::Vector b = a - 2.0f * HorizOff * GetMatrix()[X];
				
				if (pParams->ContainsFlag(Crc::ConstCRC("Left")))
        ...
        // @flag Backside |
        // @flag Frontside |
		case Crc::ConstCRC("LastSpinWas"):
			if (pParams->ContainsFlag(Crc::ConstCRC("Backside")))
			{
				if (GetFlag(FLIPPED))
				{
			...
			else if (pParams->ContainsFlag(Crc::ConstCRC("Frontside")))
			{
				if (GetFlag(FLIPPED))
				{
					return mYAngleIncreased ? CBaseComponent::MF_TRUE : CBaseComponent::MF_FALSE;
		...
		case Crc::ConstCRC("SetLandedFromVert"):
		...
		case Crc::ConstCRC("TurnToFaceVelocity"):
		...
		case Crc::ConstCRC("OverrideLimits"):
		{
			if (pParams->ContainsFlag(Crc::ConstCRC("end")))
			{
				m_override_limits_time = 0.0f;
			}
			else
			{
				pParams->GetFloat(Crc::ConstCRC("max"), &m_override_max, Script::ASSERT);
				m_override_max_max = m_override_max;
				pParams->GetFloat(Crc::ConstCRC("max_max"), &m_override_max_max);
				m_override_limits_time = 10000000000000.0f;
				pParams->GetFloat(Crc::ConstCRC("time"), &m_override_limits_time);
				m_override_air_friction = 0.0000020f;
				pParams->GetFloat(Crc::ConstCRC("friction"), &m_override_air_friction);
				m_override_down_gravity = GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Gravity"));
				pParams->GetFloat(Crc::ConstCRC("gravity"), &m_override_down_gravity);
				
				if (pParams->ContainsFlag(Crc::ConstCRC("NoTimeLimit")))
				{
		...
		case Crc::ConstCRC("SetSpecialFriction"):
		{
			...
			pParams->GetFloat(Crc::ConstCRC("Duration"), &m_special_friction_duration);
			break;
		}
		...
		case Crc::ConstCRC("OverrideCancelGround"):
			SetFlag(OVERRIDE_CANCEL_GROUND, !pParams->ContainsFlag(Crc::ConstCRC("Off")));
			break;	
		...
		case Crc::ConstCRC("SetExtraPush"):
		{
			m_wall_push_radius = 48.0f;
			m_wall_push_speed = 100.0f;
			m_wall_rotate_speed = 6.0f;
			pParams->GetFloat(Crc::ConstCRC("radius"), &m_wall_push_radius);
			pParams->GetFloat(Crc::ConstCRC("speed"), &m_wall_push_speed);
			pParams->GetFloat(Crc::ConstCRC("turn"), &m_wall_rotate_speed);
			break;
		}
		...
		case Crc::ConstCRC( "GetMatrixNormal"):
		{
			...
			pScript->GetParams()->AddFloat(Crc::ConstCRC("z"), GetMatrix()[Y][Z]);
			break;
		}
		
		// Overloading the CMotionComponent member function for skater specific logic
		case Crc::ConstCRC("Obj_MoveToNode"):
		{
			...
			if (!pParams->ContainsFlag( Crc::ConstCRC("NoReset") ))
			{
			...
			else
			{
				...
				if ( pParams->ContainsFlag( Crc::ConstCRC("orient") ))
				{
		...
		{
			pScript->GetParams()->AddVector(Crc::ConstCRC("Pos"),m_last_ground_pos[X],m_last_ground_pos[Y],m_last_ground_pos[Z]);
			break;
		}
...
void CSkaterCorePhysicsComponent::CleanUpSkateState (   )
{
	if (m_vert_air_last_frame)
	{
        ...
        GetObj()->BroadcastEvent(Crc::ConstCRC("SkaterExitVertAir"));
	}
	
	// longer rerail delay when walking
	m_rerail_time = static_cast< Tmr::Time >(GetPhysicsFloat(Crc::ConstCRC("Rail_walk_rerail_time")));
...
void CSkaterCorePhysicsComponent::limit_speed (   )
{
	...
	float max_max_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")); 
	float max_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Speed_Stat"));
	...
	if (speed > max_speed)
	{
		apply_wind_resistance(GetPhysicsFloat(Crc::ConstCRC("Physics_Heavy_Air_Friction")));
	}
...
{		
	...
	Mth::Vector gravity(0.0f, GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Gravity")), 0.0f);
	...
	switch (mp_balance_trick_component->mBalanceTrickType)
	{
		...
		case Crc::ConstCRC("Manual"):
			...
			if (Mdl::Skate::Instance()->GetCareer()->GetCheat(Crc::ConstCRC("CHEAT_PERFECT_MANUAL")))
			{
				...
				Mdl::Skate::Instance()->GetCareer()->SetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_PERFECT_MANUAL")));
		...
		case Crc::ConstCRC("Skitch"):
            ...
            if (Mdl::Skate::Instance()->GetCareer()->GetCheat(Crc::ConstCRC("CHEAT_PERFECT_SKITCH")))
			{
				// Here, set the flag. It may seem redundant, but the above line is very likely
				// to be hacked by gameshark. They probably won't notice this one, which will
				// set the flags as if they had actually enabled the cheat -- which enables us
				// to detect that it has been turned on more easily.
				Mdl::Skate::Instance()->GetCareer()->SetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_PERFECT_SKITCH")));
	...
	if (GetFlag(SKITCHING))
	{
		...
		{
			GetVel() = mp_movable_contact_component->GetContact()->GetObj()->GetVel() * GetPhysicsFloat(Crc::ConstCRC("skitch_speed_match"));
			DUMP_VELOCITY;
		}
		m_movement.Set();
	...
	#ifdef __NOPT_ASSERT__
	if (Script::GetInteger(Crc::ConstCRC("skater_trails")))
	{
...
{		
	...
	if (speed < GetPhysicsFloat(Crc::ConstCRC("Physics_Brake_Acceleration")) * 2.0f * m_frame_length)
	{
			GetVel().Set();
			DUMP_VELOCITY;
	}
	else
	{
		...
		brake.Normalize(-GetPhysicsFloat(Crc::ConstCRC("Physics_Brake_Acceleration")) * m_frame_length);
		GetVel() += brake;
		DUMP_VELOCITY;
		#endif
	}
...
{
	float speed = GetVel().Length();
	if (speed < GetPhysicsFloat(Crc::ConstCRC("Skater_max_sloped_turn_speed"))
		&& m_current_normal[Y] < GetPhysicsFloat(Crc::ConstCRC("Skater_max_sloped_turn_cosine")))
	{
		return true;
	}
	return false;
...
bool CSkaterCorePhysicsComponent::can_kick (   )
{
	...
	if (!GetFlag(TENSE))
	{
		if (speed > GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Standing_Kick_Speed_Stat")))
		{
			return false;
		}
	}
	...
	{
		if (speed > GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Crouched_Kick_Speed_Stat")))
		{
			return false;
		}
	}
...
void CSkaterCorePhysicsComponent::do_kick (   )
{
	...
	if (GetFlag(TENSE))
	{
		forward *= GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_crouching_Acceleration_stat"));
	}
	else
	{
		forward *= GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Standing_Acceleration_stat"));
	}
...
void CSkaterCorePhysicsComponent::handle_ground_friction (   )
{
	...
	if (!m_auto_kick && !GetFlag(IS_BAILING) && m_rolling_friction == GetPhysicsFloat(Crc::ConstCRC("Physics_Rolling_Friction")))
	{
		// autokick friction
		// currently none
	}
...
{	
	...
	float crouched_friction = GetPhysicsFloat(Crc::ConstCRC("Physics_Crouched_Air_Friction"));
	float standing_friction = GetPhysicsFloat(Crc::ConstCRC("Physics_Standing_Air_Friction"));
...
bool CSkaterCorePhysicsComponent::slide_off_slow_steep_slope (   )
{
	...
	if (on_steep_slow_slope())
	{
		...
		float rot = GetPhysicsFloat(Crc::ConstCRC("Skater_Slow_Turn_on_slopes")) * Mth::Sgn(angle) * m_frame_length;
...
void CSkaterCorePhysicsComponent::push_away_from_walls (   )
{
	...
	start = GetPos() + GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Skater_side_collide_height"));
...
void CSkaterCorePhysicsComponent::move_to_skitch_point (   )						
{
	...
	if (!mp_movable_contact_component->GetContact() || !mp_movable_contact_component->GetContact()->GetObj())
	{
		FLAGEXCEPTION(Crc::ConstCRC("OffMeterBottom"));
		return;
	}
	...
	if (dir != 0.0f && (!m_moving_to_skitch || dir != m_skitch_dir))
	{
		...
		while (len < max)
		{
			...
			if (index != m_skitch_index)
			{
				...
				if (dir == 1.0f)
				{
					FLAGEXCEPTION(Crc::ConstCRC("SkitchLeft"));
				}
				else
				{
					FLAGEXCEPTION(Crc::ConstCRC("SkitchRight"));
				}				
				break;
			}	
	...
	Mth::Vector target = skitch_point + GetPhysicsFloat(Crc::ConstCRC("skitch_offset")) * -p_skitch_object->GetMatrix()[Z];
...
void CSkaterCorePhysicsComponent::handle_forward_collision (   )
{
	...
	m_col_end = GetPos() + up_offset + forward * GetPhysicsFloat(Crc::ConstCRC("Skater_First_Forward_Collision_Length"));
...
void CSkaterCorePhysicsComponent::bounce_off_wall ( const Mth::Vector& normal )
{
	...
	float min = Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Wall_Bounce_Dont_Slow_Angle"))); 
	...
	if (Mth::Abs(angle) > min)
	{
		...
		if (old_speed > GetPhysicsFloat(Crc::ConstCRC("Wall_Bounce_Dont_Flail_Speed")))
		{
			#ifdef	__NOPT_ASSERT__
			{
			...
			{
				FLAGEXCEPTION(Crc::ConstCRC("FlailRight"));
			}
			...
			mp_sound_component->PlayBonkSound( old_speed / GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")), m_feeler.GetTerrain());
		}
	}
	...
	m_col_end = GetPos() + up_offset + next_movement
		+ forward * GetPhysicsFloat(Crc::ConstCRC("Skater_First_Forward_Collision_Length"));
...
void CSkaterCorePhysicsComponent::snap_to_ground (   )
{
	...
	// if (GetFlag(SKITCHING))
	// {
		// m_col_start = GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up_SKITCHING"));	// much above feet
	// }
	// else
	// {
		m_col_start = GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up"));	  		// bit above the feet
	// }
	...
	if (get_member_feeler_collision())
	{
		...
		else
		{
			...
			if (!control_pad.m_up.GetPressed())
			{
				stick_angle_cosine = cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Ground_stick_angle"))));
			}
			else
			{
				stick_angle_cosine = cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Ground_stick_angle_forward"))));
			}
			...
			else
			{
				...
				if (drop_sign > 0.0f)
				{
					...
					float min_drop = GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Down"));				
                    // if (GetFlag(SKITCHING))
					// {
						// min_drop = GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Down_Skitching"));				
					// }
	...
	{
		SetState(AIR);
		GetObj()->BroadcastEvent(Crc::ConstCRC("SkaterOffEdge"));
		FLAGEXCEPTION(Crc::ConstCRC("GroundGone"));
		...
		if (GetFlag(VERT_AIR))
		{
			...
			if (static_cast< int >(control_pad.m_up.GetReleasedTime()) > GetPhysicsInt(Crc::ConstCRC("Skater_vert_active_up_time"))
				&& static_cast< int >(control_pad.m_up.GetPressedTime()) > GetPhysicsInt(Crc::ConstCRC("Skater_vert_active_up_time")))			
			{
...
void CSkaterCorePhysicsComponent::adjust_normal (   )
{
	...
	{			
		...
		else
		{
			...
			m_normal_lerp -= GetPhysicsFloat(Crc::ConstCRC("Normal_Lerp_Speed")) * m_frame_length * 60.0f;
...
{			
	...
	if (GetFlag(LAST_POLY_WAS_VERT))	
	{
		...
		if (up_plane_normal.Length() > 0.001f)
		{
			...
			GetPos() += up_plane_normal * GetPhysicsFloat(Crc::ConstCRC("Physics_Vert_Push_Out"));
			DUMP_POSITION;
...
void CSkaterCorePhysicsComponent::maybe_break_vert (   )
{
	...
	if (
		(	
			...
			&& static_cast< int >(control_pad.m_up.GetPressedTime()) > GetPhysicsInt(Crc::ConstCRC("Skater_vert_push_time"))
	...
	)
	{
		...
		if (!BREAK_SPINE_BUTTONS)
		{
			...
			float speed = GetVel().Length() * GetPhysicsFloat(Crc::ConstCRC("physics_break_air_speed_scale"));
			
			GetVel()[X] += -m_display_normal[X] * speed;
			GetVel()[Z] += -m_display_normal[Z] * speed;
			GetVel()[Y] *= GetPhysicsFloat(Crc::ConstCRC("physics_break_air_up_scale"));
			DUMP_VELOCITY;
			...
			GetMatrix().RotateXLocal(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Skater_Break_Vert_forward_tilt"))));				
...
bool CSkaterCorePhysicsComponent::maybe_spine_transfer (   )
{
	...
	if (!hip_transfer)
	{
		GetObj()->SpawnAndRunScript(Crc::ConstCRC("SkaterAwardTransfer"));	// award a trick (might want to do it as an exception later)
	}
	else
	{
		GetObj()->SpawnAndRunScript(Crc::ConstCRC("SkaterAwardHipTransfer"));	// award a trick (might want to do it as an exception later)
	}
	
	// no late jumps during a transfer
	GetObj()->RemoveEventHandler(Crc::ConstCRC("Ollied"));
...
bool CSkaterCorePhysicsComponent::maybe_acid_drop ( bool skated_off_edge, const Mth::Vector &pos, const Mth::Vector& old_pos, Mth::Vector& vel, SAcidDropData& acid_drop_data )
{
	...
	if (mp_physics_control_component->IsWalking())
	{
		if (mp_walk_component->m_state == CWalkComponent::WALKING_GROUND)
		{
			...
			scan_distance = Script::GetFloat(Crc::ConstCRC("Physics_Acid_Drop_Walking_On_Ground_Search_Distance"));
	...
	if (skated_off_edge)
	{
		vel[Y] = Mth::Max(vel[Y], GetPhysicsFloat(Crc::ConstCRC("Physics_Acid_Drop_Pop_Speed")));
	}
	...
	{
		vel[Y] = Mth::Min(vel[Y], 2.0f * GetPhysicsFloat(Crc::ConstCRC("Physics_Acid_Drop_Pop_Speed")));
	}
	...
	SetFlagFalse(SPINE_PHYSICS);
	if (time_to_reach_target_height < Script::GetFloat(Crc::ConstCRC("Physics_Acid_Drop_Min_Air_Time")))
	{
		Nx::CCollCacheManager::sDestroyCollCache(p_coll_cache);
		vel = hold_vel;
		return false;
...
void CSkaterCorePhysicsComponent::enter_acid_drop ( const SAcidDropData& acid_drop_data )
{
	...
	p_params->AddFloat(Crc::ConstCRC("DropHeight"), GetPos()[Y] - true_target_height);
	GetObj()->SpawnAndRunScript(Crc::ConstCRC("SkaterAcidDropTriggered"), -1, false, false, p_params);
...
void CSkaterCorePhysicsComponent::handle_post_transfer_limit_overrides (   )
{
	...
	if (m_began_frame_in_transfer && !GetFlag(SPINE_PHYSICS) && !GetFlag(IS_BAILING))
	{
		...
		float standard_max = GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Speed_Stat"));
		float speed = GetVel().Length();
		if (speed < 1.25f * standard_max) return;
		
		// setup state to ignore speed limits
		m_transfer_overrides_factor = Mth::Min(speed / standard_max, GetPhysicsFloat(Crc::ConstCRC("Physics_Transfer_Speed_Limit_Override_Max")));
	...
	m_transfer_overrides_factor -= m_frame_length * GetPhysicsFloat(Crc::ConstCRC("Physics_Transfer_Speed_Limit_Override_Drop_Rate"));
	...
	m_override_max_max = m_override_max / standard_max * GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Speed_Stat"));
...
float CSkaterCorePhysicsComponent::get_air_gravity (   )
{
	...
	{							  
		// gravity = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Gravity")) / GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_vert_hang_Stat"));
		gravity = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Gravity")) / GetPhysicsFloat(Crc::ConstCRC("Physics_vert_hang_Stat"));
	}
	else
	{
		// gravity = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Gravity")) / GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Air_hang_Stat"));
		gravity = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Gravity")) / GetPhysicsFloat(Crc::ConstCRC("Physics_Air_hang_Stat"));
	}
	
	if (Mdl::Skate::Instance()->GetCareer()->GetCheat(Crc::ConstCRC("CHEAT_MOON")))
	{
		// Here, set the flag. It may seem redundant, but the above line is very likely
		// to be hacked by gameshark. They probably won't notice this one, which will
		// set the flags as if they had actually enabled the cheat -- which enables us
...
void CSkaterCorePhysicsComponent::handle_ground_rotation (   )
{
	...
	if (control_pad.m_left.GetPressed())
	{
		if (!control_pad.m_down.GetPressed())
		{
			rot = GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Rotation"));	
		}
		else
		{
			rot = GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Sharp_Rotation")); 
			if (speed < 10.0f)
			{
	...
	{
		float cess_turn_min_speed = GetPhysicsFloat(Crc::ConstCRC("cess_turn_min_speed"));
		if (speed > cess_turn_min_speed)
		{
			do_cess = true;
			
			float cess_turn_cap_speed = GetPhysicsFloat(Crc::ConstCRC("cess_turn_cap_speed"));
			...
			rot = rot * scale * GetPhysicsFloat(Crc::ConstCRC("cess_turn_multiplier"));
		}
	}
	*/
...
{				
	...
	if (speed > 0.00001f)
	{
		...
		// {
		  ...
		  // old_vel *= 1.0f - GetPhysicsFloat(Crc::ConstCRC("cess_Friction"));
		  // vel += old_vel;
		  // DUMP_VELOCITY;
		// }
	}
...
void CSkaterCorePhysicsComponent::check_side_collisions (   )
{
	...
	float side_col = GetPhysicsFloat(Crc::ConstCRC("Skater_side_collide_length"));
...
bool CSkaterCorePhysicsComponent::check_side ( float side, float side_col )
{
	...
	m_col_start = GetPos() + GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Skater_side_collide_height"));
	
	float col_len = side_col;
	#ifdef STICKY_WALLRIDES
	if (GetState() == AIR)
	{
		col_len += GetPhysicsFloat(Crc::ConstCRC("Skater_air_extra_side_col"));
	}
	#endif
	...
	#endif
	{
		...
		if (angle < 0.25f && angle > -0.25f)
		{
			...
			if (!feeler.GetCollision())
			{
				...
				GetPos() = feeler.m_end - GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Skater_side_collide_height"));
				DUMP_POSITION;
				return true;
			}
		}
...
bool CSkaterCorePhysicsComponent::check_for_wallpush (   )
{
	...
	{
		GetVel() *= Mth::Max(
			Script::GetFloat(Crc::ConstCRC("Physics_Wallpush_Min_Exit_Speed")),
			speed - Script::GetFloat(Crc::ConstCRC("Physics_Wallpush_Speed_Loss"))
		) / speed;
	}
	else
	{
		GetVel() = -Script::GetFloat(Crc::ConstCRC("Physics_Wallpush_Min_Exit_Speed")) * GetMatrix()[Z];
	}
...
bool CSkaterCorePhysicsComponent::check_for_wallplant (   )
{
	...
	// last wallplant must not have been too recently
	if (Tmr::ElapsedTime(m_last_wallplant_time_stamp) < Script::GetFloat(Crc::ConstCRC("Physics_Disallow_Rewallplant_Duration"))) return false;
	
	// not when you're too near the ground
	if (mp_state_component->m_height < Script::GetFloat(Crc::ConstCRC("Physics_Min_Wallplant_Height"))) return false;
	...
	horizontal_normal[Y] = 0.0f;
	horizontal_normal.Normalize();
	if (Mth::DotProduct(horizontal_forward, horizontal_normal) > -sinf(Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("Physics_Wallplant_Min_Approach_Angle"))))) return false;
	...
	Script::CArray *p_trick_query_array=Script::GetArray(Crc::ConstCRC("Wallplant_Trick"));
	...
	{
		GetVel() = -Script::GetFloat(Crc::ConstCRC("Physics_Wallplant_Min_Exit_Speed")) * horizontal_forward;
	}
	...
	GetObj()->SelfEvent(Crc::ConstCRC("WallPlant"));
...
bool CSkaterCorePhysicsComponent::check_for_wallride (   )
{
		...
		)) && static_cast< int >(Tmr::ElapsedTime(mWallrideTime)) > 1000 * GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Delay")))
	{
		...
		if (speed_along_wall < GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Min_Speed"))) return false;
		
		float dot = fabsf(Mth::DotProduct(SquashedVel, SquashedWallNormal));
		
		// If all angles OK then trigger a wall-ride.
		if (mWallNormal[Y] > -sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Upside_Down_Angle")))) && 
			dot < sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Max_Incident_Angle")))) && 
			GetMatrix()[Y][Y] > cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Max_Tilt")))))
		{
			if (!mp_trick_component->GraffitiTrickStarted())
			{
			...
			if (Mth::DotProduct(GetVel(), Cross) < 0.0f)
			{
				...
				FLAGEXCEPTION(Crc::ConstCRC("WallRideLeft"));
			}
			else
			{
				FLAGEXCEPTION(Crc::ConstCRC("WallRideRight"));
			}
		...
		else
		{
			...
			// FLAGEXCEPTION(Crc::ConstCRC("WallRideBail"));
		}
	}
	return false;
}
...
float CSkaterCorePhysicsComponent::rotate_away_from_wall ( const Mth::Vector& normal, float &turn_angle, float lerp )
{
	...
	turn_angle = angle * GetPhysicsFloat(Crc::ConstCRC("Wall_Bounce_Angle_Multiplier")) * lerp;
...
void CSkaterCorePhysicsComponent::check_leaning_into_wall (   )
{
	...
	if (GetVel().Length() > GetPhysicsFloat(Crc::ConstCRC("Skate_min_wall_lean_push_speed"))) return;
	...
	float min_time = GetPhysicsFloat(Crc::ConstCRC("Skate_wall_lean_push_time"));
	if (time > min_time)
	{
		time = min_time;
	}
	...
	float length = time / min_time * GetPhysicsFloat(Crc::ConstCRC("Skate_wall_lean_push_length"));	

	
	// Now we've got the length, and in the right direction, check for a collision
	
	m_col_start = GetPos() + GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Skate_wall_lean_push_height"));
...
{	
	...
	if (!GetFlag(IS_BAILING)
		...
		&& !is_braking()
		&& Mth::DotProduct(GetVel(), GetMatrix()[Z]) < 0.0f
		&& GetVel().LengthSqr() > Mth::Sqr(GetPhysicsFloat(Crc::ConstCRC("Skater_Flip_Speed")))
		)
	{
...
bool CSkaterCorePhysicsComponent::maybe_flag_ollie_exception (   )
{
	...
	if (GetFlag(TENSE) && Script::GetInteger("TurboOllie"))
	{
		...
		SetFlagFalse(TENSE);
		FLAGEXCEPTION(Crc::ConstCRC("Ollied"));
		return true;
	}
	#endif
...
void CSkaterCorePhysicsComponent::maybe_skitch (   )
{
	...
	if (p_closest)
	{
		...
		FLAGEXCEPTION(Crc::ConstCRC("Skitched"));
	}
}
...
void CSkaterCorePhysicsComponent::do_in_air_physics (   )
{
	...
	if (GetFlag(CAN_BREAK_VERT))
	{
		...
		if (static_cast< int >(Tmr::ElapsedTime(GetFlagTime(CAN_BREAK_VERT))) > GetPhysicsInt(Crc::ConstCRC("Skater_Vert_Allow_break_Time")))
		{
			SetFlagFalse(CAN_BREAK_VERT);
		}
	}
	...
	if (!mp_movable_contact_component->HaveContact())
	{
		...
		if (GetFlag(TRACKING_VERT) && GetFlag(VERT_AIR))
		{
			...
			if (collision && change_dot > 0.02f)
			{
				...
				GetPos() += m_vert_normal * (GetPhysicsFloat(Crc::ConstCRC("Physics_Vert_Push_Out")));
				DUMP_POSITION;
	...
	if (!snapped_up && get_member_feeler_collision())
	{
		...
		else
		{
			...
			if (m_col_flag_skatable)
			{
				...
				mp_sound_component->PlayLandSound(GetObj()->GetVel().Length() / GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")), mp_state_component->m_terrain);
				...
				if (mp_physics_control_component->HaveBeenReset())
				{
					...
					// Need it though?
					FLAGEXCEPTION(Crc::ConstCRC("Landed"));
					return;
				}
				...
				else
				{
					...
					else
					{
						...
						if (GetVel().LengthSqr() < Mth::Sqr(Script::GetFloat(Crc::ConstCRC("Physics_Acid_Drop_Min_Land_Speed"))))
						{
							GetVel().Normalize(Script::GetFloat(Crc::ConstCRC("Physics_Acid_Drop_Min_Land_Speed")));
						}
					}
			...
			else
			{
				...
				GetPos() += normal * GetPhysicsFloat(Crc::ConstCRC("Skater_Min_Distance_To_Wall"));
				DUMP_POSITION;
...
void CSkaterCorePhysicsComponent::handle_air_rotation (   )
{
	...
	// {
		if (m_spin_taps)
		{
			...
			{
				time = 1;
				float turn_amount = Mth::Sgn(m_tap_turns) * GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_tap_turn_speed_stat"));
		...
		else
		{
			if (control_pad.m_L1.GetPressed() && !control_pad.m_R1.GetPressed())
			{
				rot = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_rotation_stat"));	
				time = 1;
				if (control_pad.m_L1.GetPressedTime() > GetPhysicsFloat(Crc::ConstCRC("skater_autoturn_cancel_time")))
				{
					SetFlagFalse(AUTOTURN);
				}
			}								
			...
			if (control_pad.m_R1.GetPressed() && !control_pad.m_L1.GetPressed())
			{
				rot = -GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_rotation_stat"));
				time = 1;
				if (control_pad.m_R1.GetPressedTime() > GetPhysicsFloat(Crc::ConstCRC("skater_autoturn_cancel_time")))
				{
					SetFlagFalse(AUTOTURN);
				}
			}								
	...
	if (!GetFlag(NO_ORIENTATION_CONTROL))
	{
		...
		if (!time)
		{
			if (control_pad.m_left.GetPressed())
			{
				...
				if (time > GetPhysicsFloat(Crc::ConstCRC("skater_autoturn_cancel_time")))
				{
					SetFlagFalse(AUTOTURN);
				}
			}
			...
			no_time = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_No_Rotate_Time"));
			ramp_time = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Ramp_Rotate_Time"));
	...
	if (!rot && GetFlag(VERT_AIR) && GetFlag(AUTOTURN))
	{
		...
		if (angle_from_vert < Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("skater_autoturn_vert_angle"))))
		{
		...
		else
		{
			...
			rot = Mth::Sgn(angle) * GetPhysicsFloat(Crc::ConstCRC("Skater_autoturn_speed"));
...
{						
	...
	if (GetFlag(SPINE_PHYSICS))
	{
		...
		{
			float rot = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_spine_lean_stat"));
	...
	if (control_pad.m_up.GetPressed())
	{
		rot = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_lean_stat"));
	...
	float no_time = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_No_Lean_Time"));
	float ramp_time = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Ramp_Lean_Time"));
...
{						   
	...
	float rot = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_recover_rate_stat"));	
...
void CSkaterCorePhysicsComponent::rotate_upright (   )
{
	...
	{
		float rot = Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("skater_upright_sideways_speed")));
		...
		if (!m_col_flag_skatable || (dot < 0.8f && normal[Y] < 0.5f))
		{
			...
			*/
				...
				&& normal[Y] < 0.01f)					   // thing we hit is not far off vert
			{
				...
				float snap_Y = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Snap_Up"));
			...
			{
				mp_sound_component->PlayBonkSound(GetObj()->GetVel().Length() / GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")), m_feeler.GetTerrain());
			}
...
bool CSkaterCorePhysicsComponent::check_for_air_snap_up ( const Mth::Vector& start_pos )
{
	...
	float up_len = GetPhysicsFloat(Crc::ConstCRC("Physics_Air_Snap_Up"));
...
bool CSkaterCorePhysicsComponent::handle_upward_collision_in_air (   )
{
	...
	float head_height = GetPhysicsFloat(Crc::ConstCRC("Skater_default_head_height"));
	
	// ignore head collisions for a duration after wallplants
    if (Tmr::ElapsedTime(m_last_wallplant_time_stamp) <= static_cast< Tmr::Time >(Script::GetInteger(Crc::ConstCRC("Physics_Ignore_Ceilings_After_Wallplant_Duration"))))
	{
		head_height = 6.0f;
	}
...
bool CSkaterCorePhysicsComponent::handle_upward_collision_in_wallride (   )
{
	...
	m_col_end = m_col_start;
	m_col_end[Y] += GetPhysicsFloat(Crc::ConstCRC("Skater_default_head_height"));
...
void CSkaterCorePhysicsComponent::do_wallride_physics (   )
{
	...
	float Theta = GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Turn_Speed"));
	...
	m_col_end = GetPos();
	m_col_end += forward * GetPhysicsFloat(Crc::ConstCRC("Skater_First_Forward_Collision_Length"));
	...
	m_col_start = GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up"));
	m_col_start += GetPos();
	
	m_col_end = GetMatrix()[Y] * GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Down_Collision_Check_Length"));
	m_col_end += GetPos();
	...
	if (maybe_flag_ollie_exception())
	{
		...
		GetVel() += GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Jump_Out_Speed")) * mWallNormal;
		GetVel()[Y] += GetPhysicsFloat(Crc::ConstCRC("Wall_Ride_Jump_Up_Speed"));
		DUMP_VELOCITY;
...
void CSkaterCorePhysicsComponent::do_wallplant_physics (   )
{
	...
	if (Tmr::ElapsedTime(m_state_change_timestamp) > Script::GetFloat(Crc::ConstCRC("Physics_Wallplant_Duration")))
	{
		SetState(AIR);
		return;
	}
...
void CSkaterCorePhysicsComponent::maybe_stick_to_rail ( bool override_air )
{
	...
	if (Tmr::ElapsedTime(m_last_wallplant_time_stamp) < static_cast< Tmr::Time >(Script::GetInteger(Crc::ConstCRC("Physics_Wallplant_Disallow_Grind_Duration")))) return;
	...
	if (Ed::CParkEditor::Instance()->UsingCustomPark() && Tmr::ElapsedTime(m_rail_time) < m_rerail_time)
	{
		...
		min_dot = cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Rail_Corner_Leave_Angle"))));
	}
	else
	{
		min_dot = 1.0f;
...
void CSkaterCorePhysicsComponent::got_rail ( const Mth::Vector& rail_pos, const CRailNode* pNode, CRailManager* p_rail_man, bool no_lip_tricks, bool from_walk )
{
	...
	if (!pNode->GetPrevLink() && !pNode->GetNextLink())
	{
		...
		dir[Y] = 0.0f;
		dir.Normalize();
		float angle = Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("Physics_Point_Rail_Kick_Upward_Angle")));
		float c = cosf(angle);
		float s = sinf(angle);
		...
		{
			if (GetVel().LengthSqr() < Mth::Sqr(1.1f * mp_walk_component->s_get_param(Crc::ConstCRC("jump_adjust_speed"))))
			{
				GetVel().Set();
				dir = GetMatrix()[Z];
				dir[Y] = 0.0f;
		...
		GetVel() += dir * GetPhysicsFloat(Crc::ConstCRC("Point_Rail_Speed_Boost"));	// add speed boost			
		DUMP_VELOCITY;
		...
		m_rerail_time = static_cast< Tmr::Time >(GetPhysicsFloat(Crc::ConstCRC("Rail_Minimum_Rerail_Time")));
		...
		// trigger the appropriate script
		GetObj()->SelfEvent(Crc::ConstCRC("PointRail"));
	...
	float LipAllowSine = sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("LipAllowAngle"))));
	if (pNode->GetFlag(LIP_OVERRIDE))
	{
		LipAllowSine = sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("LipAllowAngle_Override"))));
	}
	float HorizSine = sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("LipPlayerHorizontalAngle"))));
	float VertCos = cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("LipRampVertAngle"))));
	...
	GetVel() += dir * sign * GetPhysicsFloat(Crc::ConstCRC("Rail_Speed_Boost"));	// add speed boost			
	DUMP_VELOCITY;
	...
	m_col_start.Set(0.0f, GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up")), 0.0f, 0.0f);
	m_col_end.Set(0.0f, -GetPhysicsFloat(Crc::ConstCRC("Rail_Bad_Ledge_Drop_Down_Dist")), 0.0f, 0.0f);
	...
	Off *= GetPhysicsFloat(Crc::ConstCRC("Rail_Bad_Ledge_Side_Dist"));
	...
	if (Mth::Abs(RightDot) < GetPhysicsFloat(Crc::ConstCRC("Rail_Tolerance")))
	{
...
void CSkaterCorePhysicsComponent::do_lip_physics (   )
{
	...
	if (mp_balance_trick_component->mBalanceTrickType == Crc::ConstCRC("Lip"))
	{
		if (Mdl::Skate::Instance()->GetCareer()->GetCheat(Crc::ConstCRC("CHEAT_PERFECT_RAIL")))
		{
			// Here, set the flag. It may seem redundant, but the above line is very likely
			// to be hacked by gameshark. They probably won't notice this one, which will
			// set the flags as if they had actually enabled the cheat -- which enables us
			// to detect that it has been turned on more easily.
			Mdl::Skate::Instance()->GetCareer()->SetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_PERFECT_RAIL")));
...
void CSkaterCorePhysicsComponent::do_rail_physics (   )
{
	...
	else
	{
		...
		// Get gravity force 	
		Mth::Vector gravity(0.0f, GetPhysicsFloat(Crc::ConstCRC("Physics_Rail_Gravity")), 0.0f);
		...
		if (sign < 0.0f)
		{
			if (pStart->GetPrevLink() && pStart->GetPrevLink()->IsActive())
			{
				...
				if (dot < cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Rail_Corner_Leave_Angle")))))
				{
			...
			if (last_segment || always_check)
			{
				...
				if (get_member_feeler_collision())
				{
					...
					if (!Ed::CParkEditor::Instance()->UsingCustomPark() || !(m_feeler.GetFlags() & mFD_INVISIBLE))
					{
						...
						FLAGEXCEPTION(Crc::ConstCRC("OffRail"));
					}
				}
			}
		}	
	...
	// if we've already started doing a trick, then start remembering our trick chain
	{
		...
		if (mp_rail_man->GetNodeArray())
		{
			Script::CArray* pNodeArray=Script::GetArray(Crc::ConstCRC("NodeArray"));
			Script::CStruct* pNode=pNodeArray->GetStructure(pRail->GetNode());
			pNode->GetChecksum(Crc::ConstCRC("name"), &m_last_rail_node_name, true);
			mp_trick_component->TrickOffObject(m_last_rail_node_name);
		}
	}
}
...
void CSkaterCorePhysicsComponent::ollie_off_rail_rotate (   )
{
	...
	{
		rot = Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Rail_Jump_Angle")));
	}
...
void CSkaterCorePhysicsComponent::skate_off_rail( const Mth::Vector& off_point)
{
	...
	float min_dot = cosf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Rail_Corner_Leave_Angle"))));
...
void CSkaterCorePhysicsComponent::maybe_trip_rail_trigger ( uint32 type )
{
	...
	uint32 value = 0;
	if (pNode->GetChecksum(Crc::ConstCRC("Triggerscript"), &value))
	{
		// we got it 
	}
	else
	{
		...
		while (pRail && pRail != pStartOfRail && pRail != p_loop_detect)
		{
			...
			if (pNode->GetChecksum(Crc::ConstCRC("Triggerscript"), &value)) break;
	...
	if (value)
	{
		...
		pNode->GetChecksum(Crc::ConstCRC("name"), &new_last_rail_node_name, Script::ASSERT);
...
bool CSkaterCorePhysicsComponent::get_member_feeler_collision ( uint16 ignore_1, uint16 ignore_0 )
{
	...
	else
	{
		...
		if (normal[Y] < sinf(Mth::DegToRad(GetPhysicsFloat(Crc::ConstCRC("Wall_Non_Skatable_Angle")))))
		{
...
void CSkaterCorePhysicsComponent::do_jump ( Script::CStruct *pParams )
{
	...
	bool play_sound = !pParams->ContainsFlag(Crc::ConstCRC("no_sound"));
	...
	switch (GetState())
	{
		case GROUND:
			...
			if (play_sound)
			{
				mp_sound_component->PlayJumpSound(GetObj()->GetVel().Length() / GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")), m_last_ground_feeler.GetTerrain());
			}	
		...
		case RAIL:
			if (play_sound)
			{
				...
				mp_sound_component->PlayJumpSound(GetObj()->GetVel().Length() / GetSkater()->GetScriptedStat(Crc::ConstCRC("Skater_Max_Max_Speed_Stat")), mp_state_component->m_terrain);
			}	
			break;
	...
	int max_tense_time = GetPhysicsInt(Crc::ConstCRC("skater_max_tense_time"));
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("BonelessHeight")))
	{
		if (from_vert)
		{
			max_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Boneless_air_Jump_Speed_stat"));
			min_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Boneless_air_Jump_Speed_min_stat"));
		}
		else
		{
			max_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Boneless_Jump_Speed_stat"));
			min_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_Boneless_Jump_Speed_min_stat"));
		}
	}	
	else
	{
		if (from_vert)
		{
			max_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_Jump_Speed_stat"));
			min_jump_speed = GetSkater()->GetScriptedStat(Crc::ConstCRC("Physics_air_Jump_Speed_min_stat"));
		}
		else
		{
	...
	GetObj()->BroadcastEvent(Crc::ConstCRC("SkaterJump"));
}			
...
void CSkaterCorePhysicsComponent::start_skitch (   )
{
	...
	mp_skitch_object->SelfEvent(Crc::ConstCRC("SkitchOn"));
}
...
void CSkaterCorePhysicsComponent::do_grind_trick ( uint Direction, bool Right, bool Parallel, bool Backwards, bool Regular )
{
	...
	Script::CArray* pMainArray = Script::GetArray(Crc::ConstCRC("GrindTrickList"));
...
void CSkaterCorePhysicsComponent::update_special_friction_index (   )
{
	...
	if (Tmr::ElapsedTime(m_special_friction_decrement_time_stamp) > static_cast< Tmr::Time >(1000.0f * Script::GetFloat(Crc::ConstCRC("Physics_Time_Before_Free_Revert"))))
	{
		m_special_friction_index--;

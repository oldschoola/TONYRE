...
CModel::CModel()
{
	...
	m_primaryMeshName = Crc::ConstCRC("Uninitialized");
...
bool CModel::AddGeom(CGeom* pGeom, uint32 geomName)
{
	...
	if ( m_numGeoms == 1 )
	{
		m_primaryMeshName = Crc::ConstCRC("TempMeshName");
	}
...
bool CModel::ReplaceTexture( uint32 geomName, const char* p_SrcTextureName, const char* p_DstTextureName )
{
	...
	if ( !success && Script::GetInteger( Crc::ConstCRC("cas_artist") ) )
	{
...
bool CModel::SetColor( Script::CStruct* pParams, float h, float s, float v )
{
	...
	uint32 geomName;
	pParams->GetChecksum( Crc::ConstCRC("part"), &geomName, Script::ASSERT );
	...
	for ( int i = 0; i < numGeoms; i++ )
	{
		...
		else if ( pParams->GetArray( Crc::ConstCRC("materials"), &pMaterialArray, Script::NO_ASSERT ) )
		{
		...
		else if ( pParams->GetChecksum( Crc::ConstCRC("material"), &mat_checksum, Script::NO_ASSERT )	)
		{
			Script::CArray* pPassArray;
			if ( pParams->GetArray( Crc::ConstCRC("pass"), &pPassArray, Script::NO_ASSERT ) )
			{
			...
			else if ( pParams->GetInteger( Crc::ConstCRC("pass"), &pass, Script::ASSERT ) )
			{
				success |= mp_geom[i]->SetMaterialColor(mat_checksum, pass, theColor);
			}
		}
		...
		if ( ( geomName == Crc::ConstCRC("skater_m_head") 
			   || geomName == Crc::ConstCRC("skater_f_head") )
			 && m_geomName[i] == Crc::ConstCRC("body") )
		{
			Dbg_MsgAssert( !mp_geom[i]->MultipleColorsEnabled(), ( "Wasn't expecting color per material on body geom" ) );
			this->ModulateColor( Crc::ConstCRC("body"), h, s, v );
		}
	}	
...
bool CModel::ApplyFaceTexture( Gfx::CFaceTexture* pFaceTexture, const char* pSrcTexture, uint32 partChecksumToReplace )
{
	...
	Script::CStruct* pModelFacePoints = Script::GetStructure( Crc::ConstCRC("original_model_face_points"), Script::ASSERT );
...
//	do_palette_gen = Script::GetInt( Crc::ConstCRC("do_palette_gen"), Script::NO_ASSERT );

...
namespace Obj
{
	...
	{
		Crc::ConstCRC("WALKING_GROUND"),
		Crc::ConstCRC("WALKING_AIR"),
		Crc::ConstCRC("WALKING_HANG"),
		Crc::ConstCRC("WALKING_LADDER"),
		Crc::ConstCRC("WALKING_ANIMWAIT")
	};
...
void CWalkComponent::Update()
{
	...
	{
		Gfx::AddDebugStar(GetObj()->GetPos(), 36.0f, MAKE_RGB(255, 255, 255), 1);
		if (m_critical_point_offset.LengthSqr() != 0.0f)
		{
			Gfx::AddDebugStar(GetObj()->GetPos() + m_critical_point_offset, 36.0f, MAKE_RGB(150, 255, 100), 1);
		}
	}
	#endif
}
...
CBaseComponent::EMemberFunctionResult CWalkComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Walk_GetStateTime"):
			pScript->GetParams()->AddInteger(Crc::ConstCRC("StateTime"), Tmr::ElapsedTime(m_state_timestamp));
			break;
		
		// @script | Walk_Jump |
		case Crc::ConstCRC("Walk_Jump"):
			jump();
			break;
		
		#ifdef SCRIPT_WALK_DRAG
		// @script | Walk_SetDragFactor |
		case Crc::ConstCRC("Walk_SetDragFactor"):
			pParams->GetFloat(NO_NAME, &m_script_drag_factor);
			break;
			
		case Crc::ConstCRC("Walk_ResetDragFactor"):
			m_script_drag_factor = 1.0f;
			break;
		#endif
			
		case Crc::ConstCRC("Walk_GetSpeedScale"):
		{
			uint32 checksum;
		...
		case Crc::ConstCRC("Walk_ScaleAnimSpeed"):
		...
		case Crc::ConstCRC("Walk_GetStateDuration"):
			pScript->GetParams()->AddFloat(Crc::ConstCRC("StateDuration"), (1.0f / 1000.0f) * Tmr::ElapsedTime(m_state_timestamp));
			break;
			
		// @script | Walk_GetPreviousState | Returns the previous state in PreviousState
		case Crc::ConstCRC("Walk_GetPreviousState"):
		{
			pScript->GetParams()->AddChecksum(Crc::ConstCRC("PreviousState"), sp_state_names[m_previous_state]);
			break;
		}
			
		// @script | Walk_GetPreviousState | Returns the current state in State
		case Crc::ConstCRC("Walk_GetState"):
		{
			pScript->GetParams()->AddChecksum(Crc::ConstCRC("State"), sp_state_names[m_state]);
			break;
		}
			
		// @script | Walk_GetHangAngle | Returns the current angle of the hang rail
		case Crc::ConstCRC("Walk_GetHangAngle"):
		{
			...
			pScript->GetParams()->AddFloat(Crc::ConstCRC("HangAngle"), Mth::RadToDeg(angle));
			break;
		}

		/*
		case Crc::ConstCRC("Walk_CancelTransitionalMomentum"):
		...
		*/
		...
		case Crc::ConstCRC("Walk_SuppressInAirControl"):
...
void CWalkComponent::ReadyWalkState ( bool to_ground_state )
{
	...
	else
	{
		...
		m_rotate_upright_timer = s_get_param(Crc::ConstCRC("rotate_upright_duration"));
	...
	else
	{
		...
		if (mp_core_physics_component->GetFlag(VERT_AIR) && m_rotate_upright_timer != 0.0f)
		{
			...
			GetObj()->GetVel() += s_get_param(Crc::ConstCRC("initial_vert_vel_boost")) * target_facing;
		}
...
void CWalkComponent::set_state ( EStateType state )
{
	...
	{
		CControlPad& control_pad = mp_input_component->GetControlPad();
		control_pad.DebounceLeftAnalogUp(s_get_param(Crc::ConstCRC("hang_control_debounce_time")));
		control_pad.DebounceLeftAnalogDown(s_get_param(Crc::ConstCRC("hang_control_debounce_time")));
	}
}
...
void CWalkComponent::go_on_ground_state (   )
{
	...
	if (m_frame_event == Crc::ConstCRC("Skid") && m_control_pegged_duration < s_get_param(Crc::ConstCRC("pegged_duration_for_skid")))
	{
...
void CWalkComponent::calculate_horizontal_speed_and_facing ( float &horizontal_speed )
{
	...
	{
		m_frame_event = Crc::ConstCRC("Run");
	}
	...
	if (m_control_magnitude > 0.0f)
	{
		...
		if ((horizontal_speed < s_get_param(Crc::ConstCRC("max_rotate_in_place_speed")) && dot < cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("max_rotate_in_place_angle")))))
			|| (horizontal_speed < s_get_param(Crc::ConstCRC("min_skid_speed")) && dot < -cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("max_reverse_angle"))))))
		{
			// low speed rotate to desired orientation with no speed change
			
			float delta_angle = Mth::DegToRad(s_get_param(Crc::ConstCRC("rotate_in_place_rate"))) * m_control_magnitude * m_frame_length;
			...
			{
				delta_angle *= s_get_param(Crc::ConstCRC("walk_rotate_factor"));
			}
			else
			{
				...
				feeler.m_start[Y] += s_get_param(Crc::ConstCRC("feeler_height"));
				feeler.m_end = feeler.m_start + s_get_param(Crc::ConstCRC("feeler_length")) * new_facing;
			...
			// setup the event
			m_frame_event = left_turn ? Crc::ConstCRC("RotateLeft") : Crc::ConstCRC("RotateRight");
		}
		else
		{
			if (dot > -cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("max_reverse_angle")))))
			{
				// if the turn angle is soft
				
				float worse_factor = Mth::Lerp(
					m_analog_control
						? s_get_param(Crc::ConstCRC("worse_worse_turn_factor"))
						: s_get_param(Crc::ConstCRC("dpad_worse_worse_turn_factor")),
					m_analog_control
						? s_get_param(Crc::ConstCRC("worse_turn_factor"))
						: s_get_param(Crc::ConstCRC("dpad_worse_turn_factor")),
					Mth::Clamp(2.0f * (1.0f - Mth::DotProduct(m_control_direction, m_facing)), 0.0f, 1.0f)
				);
				
				// below a speed threshold, scale up the turn rate
				float turn_factor;
				if (horizontal_speed < s_get_param(Crc::ConstCRC("max_pop_speed")))
				{
					// quick turn
					turn_factor = Mth::Lerp(
						s_get_param(Crc::ConstCRC("best_turn_factor")),
						worse_factor,
						horizontal_speed / s_get_param(Crc::ConstCRC("max_pop_speed"))
					);
				}
				else
				{
			...
			else
			{
				...
				if (horizontal_speed > s_get_param(Crc::ConstCRC("min_skid_speed")))
				{
	...
	{
		float decel_factor = s_get_param(Crc::ConstCRC("decel_factor"));
		
		if (desired_speed == 0.0f)
		{
			if (horizontal_speed > s_get_param(Crc::ConstCRC("walk_speed")))
			{
				m_frame_event = Crc::ConstCRC("Skid");
			}
			else
			{
				if (m_last_frame_event == Crc::ConstCRC("Skid") && horizontal_speed > s_get_param(Crc::ConstCRC("stop_skidding_speed")))
				{
					m_frame_event = Crc::ConstCRC("Skid");
				}
				else
		...
		if (mp_physics_control_component->GetStateSwitchTime() == m_state_timestamp
			...
			&& desired_speed < s_get_param(Crc::ConstCRC("walk_speed")))
		{
			decel_factor = 1.0f;
	...
	else
	{
		if (m_run_toggle)
		{
			horizontal_speed += s_get_param(Crc::ConstCRC("run_accel_rate")) * m_frame_length;
		}
		else
		{
			horizontal_speed += s_get_param(Crc::ConstCRC("walk_accel_rate")) * m_frame_length;
		}
		if (horizontal_speed > desired_speed)
		{
...
bool  CWalkComponent::adjust_horizonal_vel_for_environment (   )
{
	...
	float feeler_length = s_get_param(Crc::ConstCRC("feeler_length"));
	...
	if (m_state == WALKING_GROUND)
	{
		if (check_for_wall_push())
		{
			...
			/* no auto-hop-to-hang
			if (Tmr::ElapsedTime(m_wall_push_test.test_start_time) > s_get_param(Crc::ConstCRC("hop_delay")))
			{
				if (maybe_climb_up_ladder() || maybe_jump_low_barrier()) return false;
			}
			else
			*/
			if (Tmr::ElapsedTime(m_wall_push_test.test_start_time) > s_get_param(Crc::ConstCRC("barrier_jump_delay")))
			{
				if (maybe_climb_up_ladder() || maybe_jump_low_barrier()) return false;
			}
		}
	...
	if (m_state == WALKING_GROUND && m_ground_normal[Y] < 0.5f)
	{
		...
		push_dir[Y] = 0.0f;
		push_dir.Normalize();
		m_pos += s_get_param(Crc::ConstCRC("push_strength")) * m_frame_length * push_dir;
		DUMP_WPOSITION
	}
	...
	for (int n = 0; n < vNUM_FEELERS + 1; n++)
	{
		...
		if (mp_contacts[n].feeler.GetDist() < s_get_param(Crc::ConstCRC("push_feeler_length")) / feeler_length)
		{
			m_pos += s_get_param(Crc::ConstCRC("push_strength")) * m_frame_length * mp_contacts[n].normal;
			DUMP_WPOSITION
		}
	}
...
float CWalkComponent::adjust_desired_speed_for_slope ( float desired_speed )
{
	...
	// skip if not running
	if (desired_speed <= s_get_param(Crc::ConstCRC("walk_speed"))) return desired_speed;
...
void CWalkComponent::respond_to_ground (   )
{
	...
	CFeeler feeler;
	feeler.m_start = m_pos;
	feeler.m_start[Y] += s_get_param(Crc::ConstCRC("snap_up_height"));
	feeler.m_end = m_pos;
	feeler.m_end[Y] -= s_get_param(Crc::ConstCRC("snap_down_height"));
	...
	if (!feeler.GetCollision() || (feeler.GetFlags() & mFD_NOT_SKATABLE))
	{
		...
		m_frame_event = Crc::ConstCRC("WalkOffEdge");
	...
	if (Mth::Abs(snap_distance) > s_get_param(Crc::ConstCRC("max_unnoticed_ground_snap")))
	{
		GetObj()->SelfEvent(snap_distance > 0.0f ? Crc::ConstCRC("SnapUpEdge") : Crc::ConstCRC("SnapDownEdge"));
	}
...
void CWalkComponent::adjust_curb_float_height (   )
{
	...
	feeler.m_end = m_pos + s_get_param(Crc::ConstCRC("curb_float_feeler_length")) * feeler_direction;
    feeler.m_end[Y] += 0.5f;
	#ifdef __USER_DAN__
	if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
	{
	...
	if (feeler.GetCollision())
	{
		...
		feeler.m_start[Y] = m_pos[Y] + s_get_param(Crc::ConstCRC("snap_up_height"));
		#ifdef __USER_DAN__
		if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
		{
		...
		if (feeler.GetCollision())
		{
			...
			if (Mth::Abs(m_curb_float_height) < 0.01f && m_control_magnitude == 0.0f && m_horizontal_vel.LengthSqr() < Mth::Sqr(s_get_param(Crc::ConstCRC("min_curb_height_adjust_vel"))))
			{
			...
			else
			{
				...
				m_curb_float_height = Mth::Lerp(m_curb_float_height, appropriate_curb_float_height, s_get_param(Crc::ConstCRC("curb_float_lerp_up_rate")) * m_frame_length);
			}
...
void CWalkComponent::jump (   )
{
	...
	switch (m_state)
	{
		...
		case WALKING_LADDER:
		case WALKING_ANIMWAIT:
			strength = s_get_param(Crc::ConstCRC("hang_jump_factor"));
			break;
	}
		...
		case WALKING_AIR:
		{
			...
			if (m_control_magnitude)
			{
				float min_launch_speed = calculate_desired_speed()
					* s_get_param(Crc::ConstCRC("jump_horiz_speed")) / get_run_speed();
...
void CWalkComponent::adjust_jump_for_ceiling_obstructions (   )
{
	...
	CFeeler feeler;
	feeler.m_start = m_pos;
	feeler.m_start[Y] += s_get_param(Crc::ConstCRC("walker_height"));
...
void CWalkComponent::go_in_air_state (   )
{
	...
	// default air event
	m_frame_event = Crc::ConstCRC("Air");
	...
	if (control_pad.m_R1.GetPressed() && !m_ignore_grab_button
		...
		|| Tmr::ElapsedTime(m_state_timestamp) > s_get_param(Crc::ConstCRC("rehang_delay"))))
	{
...
void CWalkComponent::control_horizontal_vel (   )
{
	...
	float perp_desired_vel = s_get_param(Crc::ConstCRC("jump_adjust_speed")) * adjust_magnitude * Mth::DotProduct(m_control_direction, perp_direction);
	...
	perp_vel = Mth::Lerp(perp_vel, perp_desired_vel, s_get_param(Crc::ConstCRC("jump_accel_factor")) * m_frame_length);
		
	// adjust velocity parallel to jump direction
	
	// desired parallel velocity
	float para_desired_vel = s_get_param(Crc::ConstCRC("jump_adjust_speed")) * adjust_magnitude * Mth::DotProduct(m_control_direction, m_primary_air_direction);
	...
	else
	{
		...
		// exponentially approach desired velocity
		para_vel = Mth::Lerp(para_vel, para_desired_vel, s_get_param(Crc::ConstCRC("jump_accel_factor")) * m_frame_length);
	}
...
void CWalkComponent::adjust_vertical_vel_for_ceiling (   )
{
	...
	float walker_height = s_get_param(Crc::ConstCRC("walker_height"));
	...
	GetObj()->SelfEvent(Crc::ConstCRC("HitCeiling"));
}
...
void CWalkComponent::check_for_landing ( const Mth::Vector& previous_pos, const Mth::Vector& final_pos )
{
	...
	if (Mth::DotProduct(m_horizontal_vel, m_facing) > 0.0f)
	{
		...
		if (m_control_magnitude == 0.0f && m_horizontal_vel.Length() < s_get_param(Crc::ConstCRC("sticky_land_threshold_speed")))
		{
			m_horizontal_vel.Set();
		}
	}
	...
	m_frame_event = Crc::ConstCRC("Land");
}
...
void CWalkComponent::maybe_wallplant (   )
{
	...
	if (Tmr::ElapsedTime(m_state_timestamp) < Script::GetFloat(Crc::ConstCRC("Skater_Late_Jump_Slop"))) return;
	...
	*/
	...
	m_vertical_vel = s_get_param(Crc::ConstCRC("vert_wall_jump_speed")) * Mth::Lerp(
		s_get_param(Crc::ConstCRC("min_jump_factor")), 
		1.0f,
		Mth::ClampMax(mp_input_component->GetControlPad().m_x.GetPressedTime() / s_get_param(Crc::ConstCRC("hold_time_for_max_jump")), 1.0f)
	);
	...
	m_horizontal_vel = s_get_param(Crc::ConstCRC("horiz_wall_jump_speed")) * exit_dir;
	m_primary_air_direction = m_facing;
	...
	m_frame_event = Crc::ConstCRC("WallPlant");
}
...
void CWalkComponent::transition_lerp_upright (   )
{
	if (m_rotate_upright_timer != 0.0f)
	{
		float lerp_factor = Mth::LinearMap(0.0f, 1.0f, m_frame_length, 0.0f, s_get_param(Crc::ConstCRC("rotate_upright_duration")));
...
bool CWalkComponent::maybe_jump_low_barrier (   )
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (Mth::DotProduct(mp_contacts[n].normal, m_control_direction) > -cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("barrier_jump_max_angle")))))
		{
	...
	float jump_height = lowest_height + height_increment + s_get_param(Crc::ConstCRC("barrier_jump_min_clearance")) - m_pos[Y];
	...
	m_frame_event = Crc::ConstCRC("Jump");
	
	GetObj()->BroadcastEvent(Crc::ConstCRC("SkaterJump"));
	
	// stop late jumps after an autojump
	GetObj()->RemoveEventHandler(Crc::ConstCRC("JumpRequested"));
...
bool CWalkComponent::maybe_in_air_acid_drop (   )
{
	...
	m_frame_event = Crc::ConstCRC("AcidDrop");
...
bool CWalkComponent::maybe_jump_to_acid_drop (   )
{
	...
	jump_vel[Y] = s_get_param(Crc::ConstCRC("acid_drop_jump_velocity"));
...
void CWalkComponent::go_anim_wait_state (   )
{
	...
	m_frame_event = Crc::ConstCRC("AnimWait");
}
...
bool CWalkComponent::maybe_stick_to_rail (   )
{
	...
	m_frame_event = Crc::ConstCRC("Rail");
...
{
	float horizontal_reach = 1.0f + s_get_param(Crc::ConstCRC("feeler_length"));
	float vertical_height = 1.0f + s_get_param(Crc::ConstCRC("walker_height"));;
	float vertical_depth = 1.0f + s_get_param(Crc::ConstCRC("snap_down_height"));
...
void CWalkComponent::update_display_offset (   )
{
	switch (m_state)
	{
		...
		default:
			m_display_offset = Mth::Lerp(m_display_offset, 0.0f, Tmr::FrameLength() * s_get_param(Crc::ConstCRC("display_offset_restore_rate")));
			break;
	}
}
...
void CWalkComponent::adjust_pos_for_false_wall (   )
{
	...
	else
	{
		...
		if (offset > -s_get_param(Crc::ConstCRC("push_feeler_length")))
		{
			m_pos -= s_get_param(Crc::ConstCRC("push_strength")) * m_frame_length * m_false_wall.normal;
			DUMP_WPOSITION
...
void CWalkComponent::get_controller_input (   )
{
	...
	if (Mth::Abs(angle) < Mth::DegToRad(s_get_param(Crc::ConstCRC("forward_tolerance"))))
	{
		angle = 0.0f;
	}
	...
	if (control_pad.m_leftX == 0.0f && control_pad.m_leftY == 0.0f)
	{
		...
		else
		{
			...
			{
				m_control_magnitude = s_get_param(Crc::ConstCRC("walk_point"));
			}
			else
			{
...
float CWalkComponent::calculate_desired_speed (   )
{
	...
	{
		float walk_point = s_get_param(Crc::ConstCRC("walk_point"));
		if (m_control_magnitude <= walk_point)
		{
			m_run_toggle = false;
			desired_speed = Mth::LinearMap(0.0f, s_get_param(Crc::ConstCRC("walk_speed")), m_control_magnitude, 0.3f, walk_point);
		}
		else
		{
			m_run_toggle = true;
			desired_speed = Mth::LinearMap(s_get_param(Crc::ConstCRC("walk_speed")), get_run_speed(), m_control_magnitude, walk_point, 1.0f);
		}
	}
...
bool CWalkComponent::determine_stand_pos ( const Mth::Vector& proposed_stand_pos, Mth::Vector& stand_pos, CFeeler& feeler )
{
	...
	float max_height_adjustment = 2.0f * s_get_param(Crc::ConstCRC("feeler_height"));
	
	feeler.m_start = proposed_stand_pos;
	feeler.m_start[Y] += max_height_adjustment;
	feeler.m_end = proposed_stand_pos;
	feeler.m_end[Y] -= s_get_param(Crc::ConstCRC("stand_pos_search_depth"));
	if (feeler.GetCollision())
	{
...
void CWalkComponent::update_run_speed_factor (   )
{
	...
	m_run_speed_factor = Mth::ClampMin(m_run_speed_factor - s_get_param(Crc::ConstCRC("run_adjust_rate")) * Tmr::FrameLength(), 0.0f);
}
...
void CWalkComponent::smooth_anim_speed ( uint32 previous_frame_event )
{
	if (m_state == WALKING_GROUND)
	{
		...
		else
		{
			...
			if (m_frame_event == Crc::ConstCRC("Run") && m_anim_effective_speed == 0.0f)
			{
				switch (previous_frame_event)
				{
					case Crc::ConstCRC("Skid"):
					case Crc::ConstCRC("RotateLeft"):
					case Crc::ConstCRC("RotateRight"):
...
void CWalkComponent::update_anim_speeds (   )
{
	switch (m_anim_scale_speed)
	{
		...
		case HANGMOVE:
		{
			float new_anim_speed = m_anim_effective_speed / s_get_param(Crc::ConstCRC("hang_move_animation_speed"));

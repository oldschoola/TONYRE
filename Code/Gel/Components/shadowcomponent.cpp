		...
		case 0x3e84c2fd:	// simple
			{
				...
				float scale = 1.0f;
				pParams->GetFloat( Crc::ConstCRC("ShadowScale"), &scale, Script::NO_ASSERT );
...
CBaseComponent::EMemberFunctionResult CShadowComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	switch ( Checksum )
	{
		...
		case ( 0xf272c43a ): // Obj_ShadowOn
		{
			...
			{
				pParams->AddChecksum( "shadowType", Crc::ConstCRC("simple") );
			}
			...
			if ( mp_shadow )
			{
				...
				{
					float offset=0.2f;
					pParams->GetFloat(Crc::ConstCRC("Offset"),&offset);
...
{
	if ( m_shadowType == Crc::ConstCRC("simple") )
	{
...
void CShadowComponent::SwitchOnSkaterShadow()
{
	...
	pTempParams->AddFloat(Crc::ConstCRC("ShadowScale"), 1.0f);
	pTempParams->AddString(Crc::ConstCRC("ShadowModel"), "Ped_Shadow");
